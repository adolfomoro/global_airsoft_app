...
final loginValueProvider = Provider.autoDispose<String>((ref) {
  final state = ref.watch(loginFieldProvider);
  return state.value;
});

/// Selector: only watch login error
final loginErrorProvider = Provider.autoDispose<String?>((ref) {
  final state = ref.watch(loginFieldProvider);
  return state.error;
});

/// Selector: only watch login validation state
final loginIsValidProvider = Provider.autoDispose<bool>((ref) {
  final state = ref.watch(loginFieldProvider);
  return state.isValid;
});

/// Selector: only watch password value
final passwordValueProvider = Provider.autoDispose<String>((ref) {
  final state = ref.watch(passwordFieldProvider);
  return state.value;
});

/// Selector: only watch password error
final passwordErrorProvider = Provider.autoDispose<String?>((ref) {
  final state = ref.watch(passwordFieldProvider);
  return state.error;
});

/// Selector: only watch password validation state
final passwordIsValidProvider = Provider.autoDispose<bool>((ref) {
  final state = ref.watch(passwordFieldProvider);
  return state.isValid;
});

/// Selector: only watch form loading state
final loginIsSubmittingProvider = Provider.autoDispose<bool>((ref) {
...
final loginFormIsValidProvider = Provider.autoDispose<bool>((ref) {
...
final loginSubmitEnabledProvider = Provider.autoDispose<bool>((ref) {
